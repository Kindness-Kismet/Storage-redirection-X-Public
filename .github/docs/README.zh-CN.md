...
- 管理应用保存时会触发运行时热更新，正在运行的重定向应用会重新应用挂载规则。
- 模块服务运行时会监听 `config/apps` 下的手动 JSON 变更，生效配置变化后由 daemon 热更新受影响进程。
- 如果没有自动生效，可以执行模块操作 `Reload Redirect`；手动重启相关应用或重启设备仅作为兜底方式。
